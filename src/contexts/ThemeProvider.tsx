...
export const ThemeProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [effective, setEffective] = useState<'light' | 'dark'>(initial.effective)
  ...
  const toggle = useCallback(() => setTheme(effective === 'dark' ? 'light' : 'dark'), [effective, setTheme])

  const value = useMemo(() => ({ stored, effective, setTheme, toggle }), [stored, effective, setTheme, toggle])
